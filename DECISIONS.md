...
## D-22 — محاذاة الواجهة لنموذج الهوية (خلفية فاتحة + نص مارون)
- **المشكلة**: رؤوس الصفحات كانت بخلفية مارون ممتلئة بينما النموذج يحدّد
  `bg-surface`/`bg-surface-muted` للعناوين بنص `text-primary`.
- **القرار**: إعادة `HeroBand` والرئيسية والمصادقة والسقيا والتنفيذية والتذييل
  إلى سطح فاتح؛ الأحمر للأزرار النشطة/النص/الشارات فقط؛ شريط لون المشروع كتمييز لا خلفية ممتلئة.
